...
/**
 * XSS Protection - Clean HTML content (simplified)
 */
    ...
    for (const [fieldName, rules] of Object.entries(schema)) {
        ...
        switch (rules.type) {
            case 'string':
                ...
                break;
            case 'email':
                result = { isValid: isValidEmail(value), error: isValidEmail(value) ? null : 'Geçersiz email formatı' };
                break;
            case 'phone':
...
// ===================================================================
// MISSING EXPORTS - Build error'larını çözmek için
// ===================================================================

/**
 * validateInput - Smart validator for strings or plain objects
 */
export function validateInput(input, options = {}) {
    // If input is a plain object (e.g., req.query or req.body)
    if (input && typeof input === 'object' && !Array.isArray(input)) {
        const { allowedFields = [], requiredFields = [], requireSanitization = false } = options;
        ...
        // Basic sanitization pass-through
        for (const [key, value] of Object.entries(input)) {
            if (requireSanitization && typeof value === 'string') {
    ...
    // Fallback: treat as string validation (backward compatibility)
    return validateString(input, options);
}

/**
 * sanitizeInput - Alias for sanitizeText  
 */
