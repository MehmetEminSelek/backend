...
// Map Turkish PersonelRol to internal RBAC roles
const TURKISH_TO_INTERNAL_ROLE = {
    GENEL_MUDUR: ROLES.SUPER_ADMIN,
    ADMIN: ROLES.ADMIN,
    SUBE_MUDURU: ROLES.MANAGER,
    URETIM_MUDURU: ROLES.MANAGER,
    SEVKIYAT_MUDURU: ROLES.MANAGER,
    CEP_DEPO_MUDURU: ROLES.MANAGER,
    SUBE_PERSONELI: ROLES.OPERATOR,
    URETIM_PERSONEL: ROLES.OPERATOR,
    SEVKIYAT_PERSONELI: ROLES.OPERATOR,
    SOFOR: ROLES.VIEWER,
    PERSONEL: ROLES.VIEWER
};
...
function normalizeRole(role) {
    ...
    // Turkish to internal
    return TURKISH_TO_INTERNAL_ROLE[role] || ROLES.GUEST;
}
...
export async function requireAuth(req, options = {}) {
    try {
        // 1. Token Verification - Safe header access
        ...
        // Normalize role from Turkish enum to internal
        const normalizedRole = normalizeRole(payload.rol);
        const roleLevel = ROLE_LEVELS[normalizedRole] || 0;

        // 3. Check Cache First
        let userPermissions = permissionCache.get(payload.id);

        if (!userPermissions) {
            // Cache miss - load from normalized role
            userPermissions = {
            ...
            // Cache permissions
            permissionCache.set(payload.id, userPermissions.permissions, roleLevel);
        }
        ...
        if (options.role && !hasRoleOrHigher(user.rol, options.role)) {
            auditLog({
                ...
                tableName: 'API',
                recordId: req.url || 'unknown',
                description: 'Access denied: Insufficient role level',
                req: req
            });
        ...
        if (options.permissions && Array.isArray(options.permissions)) {
            ...
            if (missingPermissions.length > 0) {
                auditLog({
                    ...
                    tableName: 'API',
                    recordId: req.url || 'unknown',
                    description: 'Access denied: Multiple permissions missing',
                    req: req
                });
...
export function withRBAC(handler, options = {}) {
    return async (req, res) => {
        try {
            // Check if req object exists and has headers
            if (!req || !req.headers) {
                console.error('RBAC Error: Invalid request object', {
                    hasReq: !!req,
                    hasHeaders: !!(req && req.headers),
                    reqKeys: req ? Object.keys(req).slice(0, 10) : []
                });
